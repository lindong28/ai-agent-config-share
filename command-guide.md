...
| `/custom:simulate-user-test` | 模拟用户测试产品（web / desktop / mobile），输出 issue 清单 | 否（fan-out subagents 单次执行） |
...
5. Codex 中执行：实施 plans/<name>-<date>
```
...
3. Codex 中执行：实施 plans/<name>-<date>
```
...
```
/custom:simulate-user-test 描述需要评测的产品功能/使用方式/PRD文档
```
