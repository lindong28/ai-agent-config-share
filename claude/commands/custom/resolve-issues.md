---
name: resolve-issues
description: 围绕一个目标批量 triage 并解决项目 issue：从一批已知 issue（典型 docs/issues/*.md）按目标筛出真正该解决的，委派 agent 逐个收敛。批量处理专用——单个已知 issue 用 supervise 或 create-plan+execute-plan。
...
入口 command：把"为一个目标盘点 issue → 核实哪些真该解决 → 按依赖顺序委派合适的 agent 解决 → 新 issue 回灌本轮"压成一条命令——supervisor 与用户在"本轮目标需要解决哪些 issue"上对齐，再监督委派的 agent 把它们逐个收敛，直到 in-scope 集清空才交还用户。

## 何时使用
- 显式 `/custom:resolve-issues [--source <path>] <目标>`
- 有一批已知 issue（典型：`docs/issues/*.md`），要围绕某个目标（上线 / 某里程碑 / 某质量门）批量解决
- 不要执行：
  - 只有单个明确 issue 且已知怎么做 → 直接 `/custom:supervise --backend codex <task>`（simple）或 `/custom:create-plan` + `/custom:execute-plan`（complex）——批量 triage 是冗余
...
**关键**：核实先于纳入。陈旧项直接回写更新 issue 文件，使其反映真实状态（issue 文件是 lifecycle 文档，让它始终为真也避免下一轮重复踩坑）。
...
- **per-issue 路由**：
  - **simple**（不需要和用户对齐设计取舍）→ 直接用 codeagent-wrapper 起 codex，按 `~/.claude/commands/custom/supervise.md` §2–§4 的 spawn / 等待 / 裁决机制（引用，不复述）；spawn-prompt 的 task 槽填该 issue 经 A2 回写纠正后的描述 + 代码定位（非原始文件文本），success criteria = 该 issue 的修复 + 不引入回归；autonomous（用户已批准整批，不再 per-issue 锁 criteria）。spawn-prompt 另须明确 codex 不自行 commit、改动留 working tree 交阶段 C 统一提交（commit 归属归 supervisor，对齐 execute-plan own complex commit 的方式），使阶段 C 确定性 own 所有 simple 落地。spawn-prompt 须界定 autonomous 的边界：codex 自主定实现层选择，但遇到需与用户对齐的设计取舍须在输出中 surface、不自行拍板；supervisor 见此升级为 complex（转 `create-plan`），不让 codex 静默替用户拍设计
...
- **resolved 归档**：每个本阶段裁定修复完成的 issue，与 A2 对已修陈旧项同规则——按 `docs-organization-protocol.md` §4.8 的同一步从 domain 文件移入 `archive/closed.md`（Notes 记修复方式 + verify 证据）。in-scope 清空须同步反映在 issue 台账上，否则下一轮把已修的当 open 重 triage。
- **新 issue 回灌（闭环）**：解决过程中新产生的 issue——先按 docs-organization-protocol 归档到对应 issues 文件（agent 行为 / 工具缺口按 supervise.md §5 落 `docs/issues/general.md`，其余按 domain 落对应文件），再走 A2 同一 lens 重新 triage（分析自主）。in/out 由 A2 的 goal→consumer 判据决定，不由 consumer tag 预判——"提速 agent 工作流"类 goal 下 agent issue 反而 in：
  - **in-scope** → supervisor 默认自主纳入并排进依赖图；仅当纳入会实质改变用户批准 A3 时的合理预期才用 `AskUserQuestion` 回到用户（gate 在 materiality，不在"是新增"本身——always-ask 属过度打断；升级判据沿用 supervise §4 Dialogue facet）。典型该问：跑 `create-plan` 的新 complex 项 / 一次涌现多条显著拉大本轮规模 / in-scope 判定踩 goal 边界。反之 cheap、contained、或交付某个已批准 issue 的必要前置 → 自主纳入；模糊时 default 回到用户
...
- 因叠加前序未提交工作而整体留给用户处理的文件（见阶段 C Scope"叠在前序未提交内容之上"条）+ 其上本轮回写了什么
...
- **spawn 级不变量继承、不重写**：同 session 复用 Codex、wrapper 报错先归因 wrapper / 适配层、后台任务 + 增量轮询（含"不被动 kill"下的环境争用主动可逆干预，见 `supervise.md` §3「环境争用监测」）、语言契约（与 agent 用 English / 与用户用中文）——这些在 supervise / execute-plan 里，本 command 继承，不在此展开。
