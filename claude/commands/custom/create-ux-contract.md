---
description: 从已有产品出发，访谈用户对齐产品全貌（L1）+ 用户视角 verify（L2）+ 验收侧重，写出可被 execute-ux-contract 当输入读的验收规格。
disable-model-invocation: true
---

# create-ux-contract

入口 command：从已有产品出发，访谈用户对齐**产品全貌（L1）+ 用户视角 verify（L2）+ 验收侧重**，写出可被 execute-ux-contract 当输入读的 ux-contract.md。

ux-contract.md 是面向使用者的**验收规格**——使用者拿它来判断"这些都过了我放心上线"，不含任何实现细节 / 内部状态 / 未来路线。后续 execute-ux-contract 在这份规格框定下执行测试。

## 何时使用

- 产品需要建立系统性的 user-observable 验收规格
- 后续要跑 execute-ux-contract，需要 contract 作为测试基准
...
- **用户**：审 contract 完整性——"这些都过了我放心上线"。contract 描述不到的验收规格会被 execute-ux-contract 漏检——contract 的完整性直接决定上线保障力度
- **execute-ux-contract**：按 contract 展开测试，验证产品是否满足其中的 user-observable 验收规格

**contract.md 含什么**：

- Layer 1 (L1)：产品全貌——产品形态 + 产品类型 + 使用方式（使用者拿它做什么）
- 横切：验收侧重（L1+L2 两层影响，L3 只列方向不决策）
- Layer 2 (L2)：用户视角 verify（独立于内部实现的「算交付完成」可观测条件）。默认逐点验「功能对不对」；某些产品类型（见 `~/.claude/references/domain-registry.md`）的 L2 还含额外验收段——验整局体验而非逐点功能（judgment+evidence）
- 范围 + 硬限制 + 隐式假设

如果你发现自己开始写"内部状态机 / 代码实现 / 后端架构"——那是 L3，停下来留给 execute-ux-contract。

### 三层意识：contract 是 L1+L2 的载体

**Layer 1 (L1) — 产品全貌 + 使用方式**：产品对外暴露的全部 user-observable 能力，**以及使用者拿这个产品做什么**。"是什么"和"用来做什么"同等重要——同样是一个"AI 资讯聚合网站"，用途如果是"每日快速浏览 AI 领域新闻"，验收侧重阅读效率和新鲜度；用途如果是"深度研究 AI 技术趋势"，验收侧重内容深度和检索能力。**产品类型**（功能型 / 游戏 等，见 `~/.claude/references/domain-registry.md`）也属 L1——它决定是否需要额外的 domain 专属验收。

**Layer 2 (L2) — 用户视角 verify**（TDD-style）：从 L1 使用者视角看「产品 work」的可观测条件。这层**独立于内部实现**——使用者不关心后端怎么做，只关心产品在他们眼里是不是 work。具体形式包含：要执行的操作序列、操作后要观测的点（UI 视觉效果、文字内容、交互反馈、延迟、错误信息等）、怎么判断通过。类比 TDD 的 RED：先写"pass 长什么样"，execute-ux-contract 负责去跑。

**Layer 3 (L3) — agent-level 高效验证**：从负责执行测试的 agent 角度看，如何高效验证 L2 中的用户视角条件——包括选择交互手段（如 computer-use、agent-browser 等）以及使用用户通常不看的信息（日志、metrics、内部 API、网络请求）。**不属于 contract**，由 execute-ux-contract 在 L1+L2 框定下自主决定。contract 的 framing 说明 L3 存在是为了让 contract author 清楚边界：contract 只写用户看得到的，不写 agent 的验证手段和交互工具选择。
...
- **在 L3**：影响 execute-ux-contract 的测试资源分配——**contract 只列方向，不替 execute-ux-contract 决策**

**关键认知**：用户描述产品时**通常不会主动表达维度偏好**——验收侧重（包括"所有维度均匀覆盖"）是隐含的，需要 contract 阶段显式跟用户对齐。AIGC / UX-heavy 产品尤其常见。

**contract 是验收侧重的主谈场**——一旦在 contract 里跟用户拍下，execute-ux-contract 直接读 contract 执行，不再追问优先级。
...
落点 + 下游 execute-ux-contract 如何读：见 §3「输出：ux-contract.md」。
...
通用 lens：**"为了让 contract 能覆盖用户侧重的验收标准并对下游 execute-ux-contract 无歧义，我现在缺哪些只能用户回答的信息？"** 剩余决策都能被 agent 合理 default 时，对齐充分。

研究 / 探索 / 实际访问产品确认假设是对齐的有机组成部分——用 computer-use 使用产品、读文档、跑 probe 确认环境——能让你给出更高质量 contract 的动作都该做。

**borderline 决策的两条 path**：

- **反转成本高**（写进 contract、execute-ux-contract 跟下去再翻盘代价大）→ 立刻 AskUserQuestion
...
**对齐**：产品对外暴露的 user-observable 能力全景——产品形态、产品类型、使用形态（入口）、使用方式 (功能集合，使用者拿它做什么)、范围和限制。这是 contract 一切验收规格的反推起点。

**lens**：产品是什么形态？使用者画像是谁、怎么使用产品、拿产品用来做什么？什么明确算成功？这个定义往往主观，取决于使用者心中的使用场景，需要主动挖出来。哪些是硬限制或隐式假设？

**常见询问方向锚点**（不限于此，按 lens 展开）：

- **形态 + 入口**：Web / Mobile / Desktop / API / CLI / 混合——产品形态决定观察手段和验收维度
- **产品类型（功能型 / 游戏 …）**：产品价值主要来自"功能正确"，还是属于有专门验收逻辑的 domain（如游戏——"玩的过程"本身即价值）？**有专门处理的产品类型及其判定定义见 `~/.claude/references/domain-registry.md`**；判定为某 domain 须在 L1 显式声明——它是下游 create / review / execute 加载该类型专属验收的开关（本命令的对齐见 §2 末「domain 专属验收」）
...
**BLOCKED**：自助部署不可行 / auth 缺失 / cost-quota 限制时，通过 AskUserQuestion 让用户决策，不得静默 fall back。

**流程走通 vs 静态观察**：浏览页面 ≠ 使用产品。截图一个页面的静止状态只能确认"页面能渲染"，不能确认"流程能走通"。contract 覆盖的每条用户流程必须至少被实际走通一次——从触发动作到观察结果。如果某些页面只能通过触发流程（如发起一次生成、提交一个表单）才能到达，那必须触发流程去到达它们。

走通流程可能有副作用（消耗 API 额度、生成内容、写入数据）。这是**观察成本决策**，不是**访问决策**——不适用 BLOCKED 逻辑，应按副作用规模判断：

- **副作用仅影响本机/隔离环境且成本可忽略**（一次本地/免费 API 调用、生成一条测试数据）→ 直接执行，不需确认
- **副作用有实质成本或影响共享资源**（批量调用、写入生产数据、消耗付费额度）→ AskUserQuestion 让用户决定，不得静默降级为 docs 推断
- 不确定时，按高成本路径处理
...
**contract 只列 L3 会被影响的方向，不替 execute-ux-contract 决策**——例如"用户优先功能完整性"是已确定偏好，但 execute-ux-contract "具体怎么验证功能完整性"是 L3 的事。

### 用户视角 verify（L2）

**对齐**：从 L1 使用者视角看「产品 work」的可观测条件。独立于内部实现，应在 contract author 了解产品全貌后收敛——类比 TDD 的 RED：先写"pass 长什么样"，execute-ux-contract 去跑。
...
**产出**：registry 指明的该 domain 验收段，与逐点功能 verify 分开成段，每项 **judgment + evidence** 形态（意图 bar + 要采的证据类型）。
...
| **domain 专属验收**（若 L1 产品类型属 registry 列出的 domain）：按该 domain 原则文件产出验收段（judgment+evidence：意图 bar + 证据类型，独立成段）；功能型标 N/A + 一句理由 | 属某 domain 却只验功能没验其专属验收 / bar 由 author 替拟而非问用户 |
...
自检通过后、handoff 之前，执行 `/custom:review-ux-contract <contract path>` 进行循环审查。循环未终止不进入 handoff——review-ux-contract 的 gate 比 §3 表格更全，跳过会让下游消费者撞上未发现的 contract 缺陷。
...
下一步：
  ...
  - 执行测试：在新 session 里跑 `/custom:execute-ux-contract`
```
