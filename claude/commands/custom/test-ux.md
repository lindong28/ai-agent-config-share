...
**版本绑定不能只靠"问"**：知道"要验收哪个构建 / commit"之后，只要产品暴露版本标记（build-stamp / version 接口 / 页面内 commit 标识等）就必须用它核对"当前运行的部署 == 用户指定的候选构建"。产品未暴露任何可核对标记、或核对不上时，所有结论必须显式带上"未锚定到候选版本"这一 caveat，不得默认当前入口就是被审版本；当无法锚定使"候选构建是否通过验收"这一目标失去意义时，按 §端到端真实使用 记 BLOCKED。
...
**验收预期高于既有合同时要显式标注，不由 session 私自定级**：本轮验收预期（用户口头 + 常识）要求了产品已发布合同（ux-contract / PRD / README）未承诺的东西时，在受影响 issue 里同时引用"用户预期"与"合同原文"，点明这是 contract-gap，严重度先按用户预期给、但标注它依赖尚未确认的合同边界，交 §Handoff 升级用户裁决。（被当作合同基线的文档，其内容不再同时充当上文的"预期来源"。）这类分歧只能升级用户仲裁、不能由 session 挑一条尺子私自消化——与 `plan-execution-principles.md`「判据不可降级」同源（那条管交付方在交付阶段放松判据，此处管评估阶段预期超出合同）。反向的"产品达不到合同承诺"是普通缺陷，按 issue 字段契约引用合同原文定级即可，不走本升级。
...
| 响应速度 | "点下去之后我等了多久？是立刻有反应，还是要盯着一块不动的屏幕数一二三？" | 切换 / 筛选 / 展开 / 翻页各自的等待，冷热各一次；首次进入到能开始读之间 |
...
**Web 产品的观察方法是 BINDING 前置**：视觉效果、响应式**布局**、**响应速度**（点下去到画面更新等了多久——"响应式"指的是随宽度重排，与它无关；`page-acceptance` 与 `visual-budget` 都把响应速度判给该档），以及**内容质量与信息架构里属于「读者能不能找到并读懂他要的答案」的那部分**，按 `~/.claude/references/web-ui-observation.md` 观察——测什么、怎么测、什么算证据以该档为准。准确性、相关性、新鲜度、覆盖度仍归上表「内容质量」自己的 lens：那几项不是观察方法能判的。
...
任何标注为 contract-gap 的 issue（§建立预期：本轮验收线与既有合同背离），其严重度**不以报告为准即定稿**——主 session 必须用 `AskUserQuestion` 把"这是要修的产品缺陷，还是高于当前合同的新需求"交用户裁决，再据裁决定级；若裁为缺陷，一并确认是否同步更新 ux-contract。报告里的定级只作为升级时的初值呈现。
