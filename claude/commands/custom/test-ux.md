...
**Web 产品的观察方法是 BINDING 前置**：视觉效果、响应式，以及**内容质量与信息架构里属于「读者能不能找到并读懂他要的答案」的那部分**，按 `~/.claude/references/web-ui-observation.md` 观察——测什么、怎么测、什么算证据以该档为准。准确性、相关性、新鲜度、覆盖度仍归上表「内容质量」自己的 lens：那几项不是观察方法能判的。
