---
argument-hint: <readme-path> [max-principle-per-subagent=10]
description: 审查单个 README 文件，逐条对照 README 写作原则评审其内容质量并修复。用于你写好或改过一份 README、想在发布前审核它是否讲清了项目时（整个 docs/ 目录的跨文件一致性用 sync-docs）。
---

# review-readme

输入 (`$ARGUMENTS`)：待审 README 文件路径，可附加 `max-principle-per-subagent=N` 覆盖默认值。
...
若审查发现现有原则未覆盖某类问题，用 AskUserQuestion 把「是否改进 `~/.claude/references/readme-review-principles.md`」作为一项决策交用户拍板——principles 缺口是高杠杆发现，只在 prose 里附带提及会被略过、用户遗忘后同类坑复发。（本仓未收录专审 meta-原则的 `/custom:review-principles`，改后的 principles 文件不再另跑一轮 meta-原则审查。）
