...
- **覆盖率 / 一致性，不止存在性**：当 verify 涉及"使用者能看到 / 获得多少数据"（接入源后该源能搜到多少篇、处理 N 条产出多少、迁移 X 条目标有多少），判据别停在存在性（命中 ≥1 / 有输出）——存在性检查按设计只在数据全无时 fail、漏一半照样 pass，故要端到端 expected-vs-actual 对比，gap 须可解释。数值基线不必在 plan 阶段固定，可要求 implementer 基于真实数据动态推算。
