...
输入：待审 spec 文件路径（通常是 `./plans/<date>-<name>/spec.md`）。
...
subagent 任务：读 `~/.claude/references/spec-review-principles.md`（按优先级——编号小者胜）+ `~/.claude/references/deep-discuss-style.md` + 目标 spec 文件，按原则逐条扫报告违反/borderline。subagent 不修改文件、不发 AskUserQuestion，只输出发现报告。
...
若审查发现现有原则未覆盖某类问题，提议改进 `~/.claude/references/spec-review-principles.md`（用户决定）。改完后执行 `/custom:review-principles spec-review-principles.md` 循环审查改动——principles 文件本身也要过 meta-原则。
