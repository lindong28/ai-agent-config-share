...
静态审必做，经验实测按 skill 类型追加：

- **静态审**：执行 `/custom:review-skill <path> max-principle-per-subagent=100` 审核产出的 skill 文件；用户显式要求全面审查时省略该覆盖，沿用 review-skill 自身的默认值。**收敛性**：判断 finding 是否需修。需修 → 改 → 重审。循环到一轮无需修。
- **是否实测**：静态审证不了 skill 真能产出对的东西——skill「做没做对」有客观判据的（产物结构是否合法、能否跑通、抽取 / 分类对不对），就在 1-2 个真实 prompt 上跑一遍看实际输出，不对则回改再跑到满意；只有纯凭品味、无任何客观维度可验的（例如纯语气 / 文风改写）才静态审即可。
