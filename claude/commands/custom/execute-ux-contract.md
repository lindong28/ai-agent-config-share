...
**并行化决策（supervisor 启动前判断）**：本轮要跑的 test step 按依赖关系分组、每组起一个独立 test session 并行执行（通道按 `~/.claude/references/delegation-policy.md` §Transport selection 判）——让每条路径拿满上下文（呼应 §2 的上下文隔离）、同时缩短端到端等待。唯一硬约束是依赖独立：有前后依赖的 step（持久化后复访、叠加在一次 happy-path 产出上等）落到同一片或自带前置，拿不准是否有依赖 → 落同片；其余按功能簇切分以分配注意力。
