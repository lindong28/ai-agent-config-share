...
**各节适用面不同，按义务分而不按位置分**（位置切分随插入而失效，本文件已经失效过两次）：**巡检义务**——「为什么需要主动巡检」「怎么巡检」「删除巡检」——只管 **idle 等待期**，即已 yield turn、靠完成回调唤醒的阶段；supervisor 仍在主动轮询后台任务输出（未 yield）时不适用。**「派发前自限」与「等待器的判据与上界」按写法本身触发**，不要求是委派、也不要求已 yield。计划执行还要遵循「Plan supervisor watchdog」与「watchdog 唤醒时先判谁会去看结果」：它们处理的不是进程挂起，而是 supervisor 在代理终态之后没有继续路由的失活。**未在上面点名的各节，适用面以该节自己开头的触发条件为准**——这条兜底是有意的，否则每加一节都要回来打补丁，而漏打不会报错。
...
## 派发前自限（每个不会自己退出的后台进程都套）

**适用面：本节三条的触发条件是「进程会不会自己结束」，不是「这是不是一次委派」**——那里问两句话，**判据两句都由 `CLAUDE.md`「Background Agent 巡检与 Teammate 回收」拥有**，此处不复述：「它凭什么会退出」的处置是本节三条；「你靠什么读到那个事件」的处置在下面「等待器的判据与上界」一节。**但三条的适用域各不相同，别整体套用**：「自限超时」对任何后台进程都成立；「`</dev/null` 掐 stdin」只治**读 stdin 的阻塞**（`codex exec` prompt-as-arg 是一例，`ffmpeg` 读 stdin 是另一例，两者同因）；「里程碑 checkpoint」要有被委派方与 prompt，非委派命令无此对象。实测那次（两条 `chrome --headless --dump-dom` 空转约 50 分钟）套的是「自限超时」。

巡检若只靠 supervisor 记得建立就不可靠；把第一道防线做成**派发命令自身的自限**，不依赖注意力——前两条是命令自身的属性、每次都套；「里程碑 checkpoint」是长委派时一并写进 prompt 的损失上界：
...
## 等待器的判据与上界（停在这里、直到某件事结束）

**适用面：本节按写法本身触发**——见开头那条按义务的划分。任何"我停在这里，直到某件事结束"的写法都算，不限于 `until` / `while`：`wait`、`tail -f | grep -m1`、`sleep 30; check` 同样是等待器，不论它等的是自己的子进程、别人的进程，还是一台远端主机上的作业。

一个等待器什么时候退，**不取决于被等的命令什么时候结束，而取决于你靠什么读到那个结束**（`CLAUDE.md`「Background Agent 巡检与 Teammate 回收」里「你靠什么读到那个事件」问的就是这个，处置在本节）。

**两个方向都会坏，别只防一个**：判据恒假 → **永不退出**，安静地一直等，没有下游会发现；判据恒真、或它的前提被破坏 → **提前假完成**，下游拿着半成品继续跑。后者更坏——前者至少停在原地。

能用直接信号就别用间接判据。但**直接信号每项都有前提，前提破了它朝"提前假完成"坏**，所以下表每一项都要连着它的前提读：

| 判据 | 类别 | 前提 / 为什么不可靠 | 前提破了朝哪坏 |
|---|---|---|---|
| `wait <pid>` | 直接信号 | 必须是**本 shell 的子进程**；`run_in_background` 派发的任务不是 | 非子进程立刻返回 127，与命令真的 127 从退出码分不开 → 提前假完成 |
| `ps -p <pid>` | 直接信号 | 派发时记下的那个 pid。它判进程在不在、**不判成没成**；**前提是它会被父进程 reap**——僵尸态下 `ps -p` 与 `kill -0` 都返回 0（实测），"已结束"被读成"还在"。长窗口另防 pid 复用：记 pid 时一并记 `ps -o lstart=`，每轮比对 | 崩了算"不在" → 提前假完成且拿不到退出码；僵尸 / pid 复用 → **永不退出** |
| `kill -0 <pid>` | 直接信号 | 同上，**且须同 uid**。跨 uid 时 EPERM 与"进程不存在"**同为 rc=1**（实测 `kill -0 1` 与 `kill -0 999999` 都是 1），只有 stderr 分得开 | 跨 uid 时活进程被读成已退出 → 提前假完成。**够不到同 uid 就改用 `ps -p`**，它不受 uid 限制 |
| 管道 EOF | 直接信号 | **所有**写端关闭才到；有 fd 被 fork 继承、或上游是不 EOF 的空管道则永远不来（「派发前自限」的「`</dev/null` 掐 stdin」条就是那个形态） | 永不退出 |
| 被等命令自己的退出码（`<cmd>; rc=$?`） | 直接信号 | 你能同步等到它。**别写成 `<cmd> && …`**——`&&` 丢弃失败路径 | 拿不到时退化成下面的落标记 |
| 命令行模式匹配（`pgrep -f`、`ps \| grep`） | 间接判据 | 匹配到自己或祖先、命令行被截断，两条都随平台变（见本节末） | 两个方向都可能，见末尾 |
| 文件存在（`[ -f flag ]`） | 间接判据 | 两条前提，缺一不可。① 标记由被等的命令自己写、且**每次运行前清掉**——上一轮遗留的标记会让下一次立即命中。② **路径出现的时刻是写入开始、不是写入结束**：`> out.log` 在命令启动那一刻就建好空文件，checkpoint / 输出目录也是先建后填，所以"它在"从来不等于"它写完了"。要判完成就别判存在——完成信号只能来自**写入方自己的终止事件**：写完关闭后 `mv` 进位、最后单独写哨兵、或直接取写入进程的退出码。**别用「大小两次读数不变」**：写入方因网络 / 锁 / 上游卡顿而暂停时，两次读数同样相同，它在"写完了"与"暂时没继续长"两种状态下输出一致——与本行正要禁掉的那条判据是同一个毛病。文件大小可留作诊断读数，不能单独当完成 authority；**目录一律不能用存在性判完成**（2026-08-13 实测两次：等 `n3.json` 出现，重定向当场建的 0 字节文件立刻命中；等 checkpoint 目录数达 2，第二个目录才写了 4.8G/41G） | 遗留标记、或路径刚被创建 → 提前假完成；命令失败没人写 → 永不退出 |
| 日志里出现某行（`tail -f \| grep -m1 READY`） | 间接判据 | 先证明**这个文件确实在收该进程的输出**，再谈那行会不会打印——输出被 `tee` 到别处、多 rank 各写各的 log 时，你盯的这个文件会全程恒静，而"进程正常跑"与"进程早就崩了"在它上面读数相同（2026-08-13 实测：盯 `04_train.console.log` 等训练结束，真实输出被 tee 去了 `logs/node0.log`，被盯的文件全程只有 8 行）。写等待器前先对该路径读一次 mtime 增长。其后才是原有前提：那行只在成功路径打印；日志一静，管道再也不返回 | 文件不在输出路径上 → 恒假、永不退出；那行只在成功路径打印 → 失败时永不退出 |
| 端口 / HTTP 探活 | 间接判据 | 它测的是"有人在听"，不是"那件事做完了" | 服务起来即恒真 → 提前假完成 |

**选直接信号要能当场说出前提的读数，说不出就当间接判据用。** `wait`：你指得出 fork 它的那一行吗（`run_in_background` 派发的指不出）；`ps -p` / `kill -0`：pid 是你记下的那个吗、同 uid 吗；EOF：写端你数得过来吗。这三条前提**只能在写的时候判**——它们破掉之后的读数与正常读数撞车（`wait` 的 127、`kill -0` 的 1），运行时分不出来。

**够不到直接信号时**（被等的命令不是你的子进程、也拿不到 pid），让它自己落一个标记，再等那个标记。**别写成 `<cmd> && echo done > flag`**：`&&` 只在成功路径写，命令失败就永远没有标记、等待器永不退出；而上一轮遗留的标记会让下一次立即假完成。可用形态是先清、无论成败都写、并把退出码交出去让人消费：

```bash
flag="${flag:?}.$$"                      # 唯一名放块内：并发实例共用一个 flag 会互相删/互相误判
: "${N:?}"; case "$N" in ''|*[!0-9]*|0) echo "N 须为正整数" >&2; exit 1;; esac
rm -f "$flag" && [ ! -e "$flag" ] || { echo "清不掉旧标记" >&2; exit 1; }
{ <cmd> && rc=0 || rc=$?; echo "$rc" > "$flag"; } </dev/null &   # && ||：set -e 下也要写
bg=$!; i=0; while [ ! -s "$flag" ] && [ "$i" -lt "$N" ]; do sleep 5; i=$((i+1)); done
[ -s "$flag" ] || { kill "$bg" 2>/dev/null; echo "TIMEOUT $((N*5))s" >&2; exit 1; }
rc=$(cat "$flag"); case "$rc" in ''|*[!0-9]*) echo "标记不是退出码:[$rc]" >&2; exit 1;; esac
[ "$rc" = 0 ] || { echo "被等的命令失败 rc=$rc" >&2; exit "$rc"; }
```

**这是形态示例，不是可以直接粘的作业运行器。** 每一行都挡一个实测过的失败（唯一名挡并发互删、`case` 挡 `$N` 非正整数时 BSD 跑两轮而 GNU 跑零轮、清标记失败挡陈旧标记造成的立即假完成、`&& ||` 挡 `set -e` 下失败命令根本不写标记、`</dev/null` 挂在整组上挡 zsh 不给后台作业接 /dev/null、`kill "$bg"` 收掉超时后的直接子进程、两个 `case` 挡非数字 rc 让 `exit` 静默失效）。**但它仍然不覆盖**：`<cmd>` 是管道时 `$?` 只是最后一段的状态（要么 `set -o pipefail`，要么先把管道包成脚本）；`<cmd>` 自己 fork 出后台子进程时，rc=0 不构成完成证据，且**超时那行的 `kill "$bg"` 收不掉孙进程**——它们会继续持有 `.output` 的写句柄，让整个后台任务永远不算完成（实测：一次 `sleep 60` 孙进程把前台调用卡满 2 分钟超时）。要 `setsid` 起、`kill -- -<pgid>` 按进程组回收；交互式 shell 里 `${var:?}` 不终止。**照抄前先对着这三条判一次你的 `<cmd>` 属不属于它们**——判得出属于，这个形态就不适用，别硬套。

最后那半句不是可选的：**等待器退出只说明"等完了"，不说明"成了"**，`rc` 不被消费时这个形态就退化成上面说的更坏的那个方向。标记文件名带上本次运行的唯一标识，别复用固定名。

**改不动被等的命令**（东西已经在跑了）：`pgrep` **只跑一次**取 pid，之后一律 `ps -p <pid>`。这把模式匹配从"每轮重判"降成"一次取值"——取空或取到多个当场就看得见。**取值那一次要排除自己和祖先**（Linux 上它们必然在结果里，见本节末），否则会认领到发起自己的那个 shell、`ps -p` 永远为真。同类写法见「卡死处置」的先认对进程。

**远端作业**：前四项直接信号是本地进程原语，跨不过主机边界——**退出码除外**，`ssh <host> '<cmd>'` 会把它透传回来，够得着就用它，这是远端最干净的形态。等不到（作业已在远端后台跑）才退回落标记 + 轮询：`until ssh <host> test -s <flag>`。上面那三条约束**一条不减，且清标记要清在远端**（`ssh <host> rm -f <flag>`；本地 `rm` 删不到它，遗留标记会让第一次探测就假完成），退出码照样写进标记（`ssh` 只回传 `test` 的结果，远端作业成没成它不知道）。ssh 自身错误固定返回 **255**、远端 `test` 未命中返回 **1**：只有 1 是"还没好"，连续读到 255 说明通道断了，立即报 transport 失败，别等满上限再报成"作业超时"。

**等待上限（无条件，与判据无关）**：等待器自身必须有限。免掉的是"判据不可靠"，不是"可以无限等"——「它凭什么会退出」答得出，免的也只是"它自己会退"，而本文件存在的理由正是它有时不退。**到点要落在与成功不同的读数上，不能只打印一行字**：循环耗尽后 `$?` 是循环体最后一条命令的残值、不是超时信号（实测同一写法在两台机器上分别给出 0 和 1），所以循环后必须补一条显式判定——上面那个片段的第 4 行就是它。计数别用 `for _ in $(seq "$N")`：`$N` 未设时 BSD 的 `seq ""` 给出 `1 0`（跑 2 次）、GNU 直接报错（跑 0 次），同一条写法两个平台的上界不同；用上面的 `while` 计数器。`timeout` 也不能顶替：它是 GNU coreutils，**macOS 上没有**（实测 `timeout` / `gtimeout` 均 command not found，缺失时返回 127、看起来像被等的命令失败了）。**harness 自己的 `timeout_ms` 更不能顶替**——`run_in_background` 派发的任务不会因它到点而终止：2026-08-13 两次实测分别声明 20 min / 30 min，实际空转 **49.6 min / 59.1 min**，最后都是人为 `TaskStop` 收的。它是这一栏里最顺手、也最容易被当成"我已经套了上界"的那个，而它恰恰把无条件规则变成可以自认为满足却实际为空。**上限只能写进被派发的脚本本身。**

命令行模式匹配是间接判据里最容易看走眼的一种，两条失效路径互相独立：

- **它会匹配到自己或自己的祖先，而这随平台变。** Linux（procps）不排除祖先：`bash -c 'until ! pgrep -f TOKEN; …'` 里 TOKEN 就在父 bash 的 argv 里，于是单个等待器就确定性永不退出（2026-08-19 在 gpu-box 实测，`pgrep -f` 返回三个 pid，其中一个正是发起它的那个 bash）。macOS/BSD 的 `man pgrep` 明写默认排除自身及全部祖先，所以同一条命令在 mac 上要两个同 pattern 等待器同时起才卡住、错开一秒则都正常退出（同日实测）。**别把在 mac 上没复现当成它不成立**——远端主机通常是 Linux。
- **命令行会被截断。** 调用 shell 的 cmdline 实测被截在 998 字节；模式落在截断点之后时对 `pgrep` 完全不可见——同一条命令于是在环境前缀短的 session 里永不退出、在长的里"正常"。

触发本节的事故与完整取证见 `~/research/ai-agent-config/docs/issues/harness-issues.md` 的 HARNESS-095。
...
- **不另建巡检**——主动轮询本身就是巡检。本文件开头那条巡检义务在 supervisor 仍在轮询、未 yield 时**不适用**（见开头那条按义务的划分）；再建一层只是重复烧 turn，而每次触发都是一个完整 turn。
...
```bash
codeagent-wrapper --progress --backend <原 backend> resume <session_id> - <workdir> <<'EOF'
<续做指令>
EOF
```

`-` 是"从 stdin 读任务"的标记而非占位符，**必须配 heredoc**（漏掉即卡在读 stdin，机制见「派发前自限」的「`</dev/null` 掐 stdin」条）；`--backend` 默认 `codex`，原任务若是别的 backend 必须显式带上。`<workdir>` **三个 backend 都承重**（codex 经 `-C`，claude / gemini 经进程 cwd）。漏掉它不是中性的：被续跑的 agent 会静默继承调用方的 cwd 且不报错——**委派目标是 git worktree 时这意味着它跑到主 checkout 上去**。
...
到终态时**按建立时记的台账逐个对账**，不是删"此刻想得起来的那几个"。差别不在勤勉：回收注意力天然由噪声驱动——它在刷屏、它误报了、要换过滤器形态——而一个**对象已经终结的巡检从此完全安静**，于是最该删的那个恰好最不容易被想起（实测一次收口：8 个巡检删了 6 个，漏下的 2 个都是对象早已完成、此后零事件的，空转 4 小时后由用户发现）。

**所以台账要在建立巡检的同一刻记下**（同上「机制」条要求的"派发的同一轮就建立"）——它是唯一完整的清单。台账缺失时只能按各机制自己的权威清单查（cron 查 `crontab` / `launchctl`，harness 起的查它自己的任务枚举）；**没有权威清单可查的那一类，写明"无法证明已清空"**，不要用 `ps | grep` 一类全文匹配冒充完整枚举：它对 durable cron 结构性失明（两次触发之间根本没有常驻进程，而 cron 恰是最需要删的一类），又会把同机其它 session 的进程、人工排查命令乃至 grep 自身一并列入，读者据此动手就是误杀。`bg-shell-reclaim-check` 同样不兜这一类：它按设计只认 `.output` 的**写**持有者，排除 `tail -f` 是为了不把旁观者误报成"任务还在跑"——**不得放宽那条判据来覆盖巡检；另建独立的巡检登记与检测不在此排除之列**。
...
CODEX_SANDBOX=read-only ~/.claude/bin/codeagent-wrapper --progress --backend codex - <workdir> <<'EOF'
<prompt>
EOF
