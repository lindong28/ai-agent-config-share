...
两条路径共用一条**L1/L2 成对写入不变量**（L1 = 产品行为承诺，L2 = 用户视角可观测的验证条件，定义与审查侧见 `ux-contract-review-principles.md`）：新增或修改任一 L1 承诺时，同一次编辑内写出其对应的 L2 验证条件；给不出验证条件的承诺不进 L1——只写承诺不写验证，该功能整体回退，契约仍会通过，等于没有防回归。既有 L2 仍能验证修改后承诺的措辞性修改视为已配对。Fallback 候选提议承诺变更时，验证条件随条目 Recommendation 给出。此处是该不变量的创作侧：靠事后审查或自查记性补配对不是机制。
