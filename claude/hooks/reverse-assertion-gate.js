...
function judge(lastMsg) {
  const prompt =
    ...
    "  已分开 → ②不成立、判 ok：为排除竞争解释另跑了一次（拿到相反面的正读数、换一种形态复跑、直接读到那个值）。**此时即便结论是反向的、即便随后把行动项交给用户，也放行——先验证再下反向结论正是本闸要奖励的行为。**（这句只赦免②这一轴。那次验证若只证到某个具体手段用不了、而结论下在它服务的目标上，仍按②′判——两条是并列分支，②不成立不能替②′作答。）\n" +
    ...
    "②′ 依据**有**区分力，但它证的是**某个手段用不了**，说出口的却是**那件事做不到**。轴只有一条：" +
    "读数作用在一个具体手段上（某个探针 / 库 / 模型 / 命令 / 接口 / 做法），而结论把它升级成那个手段" +
    "服务的**目标**不可达、这件事没法做、这一类测不了。\n" +
    "  **这一态最容易放过去，因为证据往往格外硬**——穷尽扫描、100% 命中、正反对照齐全。但那份硬只证明" +
    "**这个手段**在这里行不通，从不证明**没有别的手段**；读数越硬，越容易让人只看它够不够硬，而略过" +
    "「它证的是手段还是目标」。\n" +
    "  两种情形**不算**②′、判 ok：读数直接作用在结论所述的那件事本身（要知道 X 在不在，就去看了 X " +
    "在不在；要知道这台机器有没有装 Y，查的就是 Y）；或结论已经把范围收在那个手段上（「这个探针在这批" +
    "素材上不可用」而不是「这件事测不了」）。**别靠联想去构造手段与目标的关系**——只有这段话自己把" +
    "结论下在比读数更高一层的东西上时，②′才成立。\n\n" +
    "判 ok（任一成立即 ok）：\n" +
    "• **依据有区分力**：做过针对这条断言的检查，其输出在断言为假时会不同——直接读到那个配置 / 值、" +
    "实际调用拿到返回、换一种形态复跑做了对照、在与结论范围相符的面内搜索过。" +
    "（依据虽有区分力、但只证到某个手段用不了而结论下在目标上的，走②′，不从这条放行。）\n" +
...
function main() {
  ...
  process.stderr.write(
    `[REVERSE-ASSERTION-GATE] 这一停把一条反向断言当结论交付了：${concern}\n` +
      ...
      "**先看拦的是哪一条。** 若上面那句 concern 说的是**射程**——你的读数确实有区分力，只是它证到的是" +
      "某个手段用不了、而结论下在了那个手段服务的目标上——那么前一段那句「代理判据」不适用，你的读数没问题，"+
      "出路是第 5 条；**别去重取读数**，也别把已经站得住的手段级结论降级成「未核实」。其余情形走前四条。\n" +
      "选一条：\n" +
      ...
      "4. 这条断言依赖的是**你自己建立的前提**，而同一诊断你已换过两次假设仍未收敛——起一次只读的独立" +
      "第二意见（`general-purpose-readonly`，**别把本线程的结论写进 prompt**，只给原始读数与问题）。" +
      "前三条都要你再取一次读数，而当错的正是前提本身时，你取什么读数都会自洽。\n" +
      "5. **把结论收回到你实际测过的那个手段上**——「这个探针 / 这条路在这批输入上不可用」，而不是" +
      "「这件事做不到」；同一目标下你没测过的其它手段，逐个点名说明本轮未测。手段级结论原样保留，" +
      "它是有效产出；被收回的只是那一步外推。\n" +
