#!/usr/bin/env node
/**
 * User-scope hook dispatcher.
 *
 * Adapted from ECC's run-with-flags.js for ~/.claude/hooks/.
 * Executes a hook script only when enabled by profile flags.
 *
 * Usage:
 *   node run-with-flags.js <hookId> <scriptRelativePath> [profilesCsv]
 *
 * Fast path: if the target script exports run(rawInput, options),
 * it is called in-process via require() (~50-100ms savings per hook).
 * Legacy path: scripts without run() are spawned as child processes.
 */
...
async function main() {
  ...
  const scriptPath = path.resolve(hooksRoot, relScriptPath);

  // Prevent path traversal outside the hooks root
  if (!scriptPath.startsWith(hooksRoot + path.sep) && scriptPath !== hooksRoot) {
  ...
  const result = spawnSync(process.execPath, [scriptPath], {
    ...
    env: {
      ...process.env,
      HOOK_INPUT_TRUNCATED: truncated ? '1' : '0',
      HOOK_INPUT_MAX_BYTES: String(MAX_STDIN),
      // ECC compat
